...
//查询多行
func (mydb *ContactDB)queryMulti() string{
	var rtnMsg string
	rtnMsg+="{"
	msg := new(Message)
	...
	for rows.Next() {
		...
		if(msg.id>=0) {
			print(msg.id)
			rtnMsg+=strconv.Itoa(msg.id)
		}
		rtnMsg+=","+string(msg.Name.String)+","+string(msg.Email.String)+","+string(msg.Text.String)+"},"
	}
	rtnMsg=rtnMsg[:len(rtnMsg)-1]
	rtnMsg+="}"
	return rtnMsg
